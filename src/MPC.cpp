...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  {
    vars_lowerbound[i] = -100000;
    vars_upperbound[i] = 100000;
  }
  ...
  for(int i = psi_start+1;i<v_start;i++)
  {
    vars_lowerbound[i] = -4*M_PI;
    vars_upperbound[i] = 4*M_PI;
  }  
  vars_lowerbound[v_start] = v;
  vars_upperbound[v_start] = v;
  for(int i = v_start+1;i<cte_start;i++)
  {
    vars_lowerbound[i] = -60;
    vars_upperbound[i] = 60;
  }  
  vars_lowerbound[cte_start] = cte;
  vars_upperbound[cte_start] = cte;
  for(int i = cte_start+1;i<epsi_start;i++)
  {
  ...
  for(int i = epsi_start+1;i<delta_start;i++)
  {
    vars_lowerbound[i] = -4*M_PI;
